...
DEFAULT_SOURCES = ('crtsh,duckduckgo,bing,otx,hackertarget,rapiddns,anubis,'
                   'threatminer,urlscan,certspotter')
...
def run(domain, sources=DEFAULT_SOURCES, out_dir=None):
    python = venv_python()
    command = [str(python), '-m', 'theHarvester', '-d', domain, '-b', sources]
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)
        command.extend(['-f', os.path.join(out_dir,
                                            'theharvester_%s' % domain.replace('/', '_'))])
